...
ConcreteQuditOp(create) ConcreteQuditOp(annihilate)
...
void oneQuditApply(QuditArgs &...args) {
  ...
  // std::size_t Levels = (args.n_levels() + ...);
...
// /// This function will apply a multi-controlled operation with the given
// control
// /// register on the single qubit target.
// template <std::size_t Levels, typename QuantumOp, typename mod = ctrl,
//           typename QuditRange>
//   requires(std::ranges::range<QuditRange>)
// void oneQuditApplyControlledRange(QuditRange &ctrls, qudit<Levels> &target) {
...
//                  [](auto &q) { return cudaq::quditToQuditInfo(q); });

//   // Apply the gate
//   getExecutionManager()->apply(gateName, {}, controls,
//                                {cudaq::quditToQuditInfo(target)});
// }
...
#define CUDAQ_QIS_PHOTONIC_ONE_TARGET_QUDIT_(NAME)                             \
  ...
  // template <std::size_t Levels, typename mod = ctrl, typename QuditRange>      \
  //   requires(std::ranges::range<QuditRange>)                                   \
  // void NAME(QuditRange &ctrls, qudit<Levels> &target) {                        \
  //   oneQuditApplyControlledRange<qudit_op::NAME##Op, Levels, mod>(ctrls,       \
  //                                                                 target);     \
  // }                                                                            \
...
// template <typename QuantumOp, typename... QuditArgs>
// void oneQuditWithControlsApply(QuditArgs &...args) {
//   // Get the name of this operation
//   auto gateName = QuantumOp::name();

//   // Get the number of input qudits
//   constexpr std::size_t nArgs = sizeof...(QuditArgs);
//   constexpr std::size_t Levels = (args.n_levels() + ...);

//   // Map the qudits to their unique ids and pack them.
//   std::vector<QuditInfo> quditInfos{quditToQuditInfo<Levels>(args)...};
//   std::vector<bool> quditIsNegated{quditIsNegative<Levels>(args)...};

//   // If we are here, then `mod` must be control or adjoint. Extract the
//   controls
//   // and the target
//   std::vector<QuditInfo> controls(quditInfos.begin(),
//                                   quditInfos.begin() + nArgs - 1);
...
//   // TODO: do we need control negation handling for qudits?
//   // If we have controls, check if any of them are negative controls, and if
//   so
//   // apply an x.
//   if (!controls.empty())
//     for (std::size_t i = 0; i < controls.size(); i++)
...
// template <typename QuantumOp, typename... QuditArgs>
// void oneQuditWithAdjointControlsApply(QuditArgs &...args) {
//   // Get the name of this operation
//   auto gateName = QuantumOp::name();
...
// template <size_t Levels, typename QuantumOp, typename QuditRange>
// void oneQuditApplyControlledRange(QuditRange &ctrls, qudit<Levels> &target) {
//   // Get the name of the operation
//   auto gateName = QuantumOp::name();
...
#define CUDAQ_QIS_PHOTONIC_ONE_TARGET_QUDIT_(NAME)                             \
  ...
  // void c##NAME(qudit &ctrl, qudit &target) {                                   \
  //   oneQuditWithControlsApply<qudit_op::NAME##Op>(ctrl, target);               \
  // }
...
// Usage: create<Levels>(qudit<Levels>&), annihilate<Levels>(qudit<Levels>&)
...
//   std::vector<QuditInfo> controls(targets.begin(), targets.begin() + nArgs -
//   1);
...
//       "swap", {}, controls, {qubitToQuditInfo(src),
//       qubitToQuditInfo(target)});
// }
...
//   std::transform(qubits.begin(), qubits.end(),
//   std::back_inserter(quditInfos),
...
// /// @brief Apply a general Pauli rotation with control qubits and a variadic
// set
...
// /// Helper function to extract a slice of a `std::vector<T>` to be used
// within
// /// CUDA-Q kernels.
// #if CUDAQ_USE_STD20
// template <typename T>
...
//       tuple_slice_last<sizeof...(Args) -
//       NUMP>(std::forward_as_tuple(args...)));
// }

// template <typename T, typename... RotationT, typename... QuantumT,
//           std::size_t NumPProvided = sizeof...(RotationT),
//           std::enable_if_t<T::num_parameters == NumPProvided, std::size_t> =
//           0>
...
// /// the number of target qubits, the number of rotation parameters (can be
// 0),
